...
@NamedQueries({
    ...
    , @NamedQuery(name = "Usefulness.findByUser1", query = "SELECT u FROM Usefulness u WHERE u.usefulnessPK.user1 = :user1")
...
public class Usefulness implements Serializable {
    ...
    @JoinColumn(name = "user_1", referencedColumnName = "id", insertable = false, updatable = false)
    @ManyToOne(optional = false)
    private Users users;
    ...
    public Usefulness(int review, int user1) {
        this.usefulnessPK = new UsefulnessPK(review, user1);
    }
