...
public class UsefulnessFacadeREST extends AbstractFacade<Usefulness> {
    ...
    @PersistenceContext(unitName = "com.mycompany_AppArkServerMaven_war_1.0-SNAPSHOTPU")
    private EntityManager em;

    private UsefulnessPK getPrimaryKey(PathSegment pathSegment) {
        /*
         * pathSemgent represents a URI path segment and any associated matrix parameters.
         * URI path part is supposed to be in form of 'somePath;review=reviewValue;user=userValue'.
         * Here 'somePath' is a result of getPath() method invocation and
         * it is ignored in the following code.
         * Matrix parameters are used as field names to build a primary key instance.
         */
        entity.UsefulnessPK key = new entity.UsefulnessPK();
        ...
        if (user != null && !user.isEmpty()) {
            key.setUser(new java.lang.Integer(user.get(0)));
        }
        return key;
    }
    ...
    public void remove(@PathParam("id") PathSegment id) {
        entity.UsefulnessPK key = getPrimaryKey(id);
